...
- `Views::Trades::Show` renders the trade negotiation page: two user cards (in-trade zone + available pool), sticker chips with add/remove forms, status badges, receipt confirmation section, accept/reject actions. Uses `editable?` helper to conditionally hide interactive elements when the current user has accepted.
